...
class CityViewController: UIViewController {
    ...
    internal var selectedCityDriver: Driver<City?> {
        return selectCitySubject.asDriverOnErrorJustComplete().do(onNext: { [weak self] _ in
    ...
    @IBAction private func selectBekasi(_ sender: Any) {
        selectCitySubject.onNext(City(id: "1", name: "Bekasi"))
    }
    
    @IBAction private func selectJakarta(_ sender: Any) {
        selectCitySubject.onNext(City(id: "2", name: "Jakarta"))
    }
    
    @IBAction private func selectSemarang(_ sender: Any) {
        selectCitySubject.onNext(City(id: "3", name: "Semarang"))
    }
}
